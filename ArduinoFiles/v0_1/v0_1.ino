...
void setup() {
  ...
  Serial.begin(115200);  // 9600 was a mistake, Too slow.
  // Motor Pin Map
  ...
  TCCR1A = 0;
  TCCR1B = 0;
  timer1_counter = 63036;   // preload timer 65536-16MHz/256/2Hz (34286 for 0.5sec) (59286 for 0.1sec) (63036 for 0.04s)
...
//  Serial.println(pwm_pulse2);
  ...
  // Fool speed statements
  if (fm)
  {
    analogWrite(m1p, fspeed1);
    analogWrite(m2p, fspeed2);
  }
...
void driveee (int spe,int drz)
{
  Serial.print("I am in driving");
  give_speed(1,spe,0);
  give_speed(2,spe+50,0);
  while (encoder1a <drz & encoder2a < drz)
    Serial.println("Drive Bitxh");
  stop();
}
